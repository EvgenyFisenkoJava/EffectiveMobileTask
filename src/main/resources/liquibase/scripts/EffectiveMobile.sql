...
alter table if exists company add constraint FK8komo0vj792gsn10xbsjo3pe7 foreign key (logo_id) references image;
...
alter table if exists purchase_history add constraint FKf1t3bdc1864c5onk6pt79pxny foreign key (product_id) references product;
...
alter table company add column active boolean;
alter table company add column user_profile_id int4;
alter table if exists company add constraint FKghawd5rtv8ok565nwpdyyute8 foreign key (user_profile_id) references user_profile;
alter table company drop column logo_id cascade ;